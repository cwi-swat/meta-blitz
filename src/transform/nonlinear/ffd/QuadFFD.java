...
public class QuadFFD implements IDeform, ILineTransformer {
	...
	private QuadFFD(List<Double> gridX, List<Double> gridY,
	...
	public static Vec evaluateQuadBezierSurface(Vec d,
			List<List<Vec>> controlPoints) {
	...
	private static double[] getQuadBernstein(double x) {
		...
		double rx = 1.0 - x;
		double rx2 = rx * rx;
		return new double[] { rx2, 2 * x * rx, x2 };
	}
	...
	public static QuadFFD makeQuadFFD(double fadeDist, double[] gridX,
			double[] gridY, Vec[][] cps) {
		List<Double> lgridX = makeCompleteGrid(fadeDist, gridX);
		List<Double> lgridY = makeCompleteGrid(fadeDist, gridY);
		List<List<Vec>> lcps = makeCompleteControlPoints(fadeDist, cps, gridX,
				gridY);
	...
	private static List<List<Vec>> makeCompleteControlPoints(double fadeDist,
		...
		res.add(empty);
		res.add(empty);
		List<Vec> first = makeNormalCollumn(fadeDist, gridY[0] - fadeDist,
				gridX);
		res.add(first);
		res.add(interpolateCollumns(first,
				makeCollumn(fadeDist, gridY[0], gridX, cps[0])));
		...
		List<Vec> end = makeNormalCollumn(fadeDist, gridY[gridY.length - 1]
				+ fadeDist, gridX);
	...
	private static List<Vec> makeNormalCollumn(double fadeDist, double y,
			double[] gridX) {
		List<Vec> res = new ArrayList<Vec>();
		res.add(Vec.ZeroVec);
		res.add(Vec.ZeroVec);
	...
	private static List<Vec> makeCollumn(double fadeDist, double y,
			double[] gridX, Vec[] vecs) {
		List<Vec> res = new ArrayList<Vec>();
		res.add(Vec.ZeroVec);
		res.add(Vec.ZeroVec);
