...
public class CubicFFD implements IDeform, ILineTransformer{
	...
	public CubicFFD(double fadeDist, List<Double> gridX, List<Double> gridY,
		...
		CubicFFDPadder pad = new CubicFFDPadder(gridX, gridY, controlPoints, fadeDist);
		pad.padAll();
...
//		return c;
	...
	public static Vec evaluateCubicBezierSurface(Vec t, List<List<Vec>> controlPoints) {
		...
		controlPoints.get(0).get(2).mul(bus[2])).add(
		controlPoints.get(0).get(3).mul(bus[3]))).mul(bvs[0]);
		Vec b = controlPoints.get(1).get(0).mul(bus[0]).add(
		controlPoints.get(1).get(1).mul(bus[1]).add(
		controlPoints.get(1).get(2).mul(bus[2])).add(
		controlPoints.get(1).get(3).mul(bus[3]))).mul(bvs[1]);
		Vec c = controlPoints.get(2).get(0).mul(bus[0]).add(
		controlPoints.get(2).get(1).mul(bus[1]).add(
		controlPoints.get(2).get(2).mul(bus[2])).add(
		controlPoints.get(2).get(3).mul(bus[3]))).mul(bvs[2]);
		Vec d = controlPoints.get(3).get(0).mul(bus[0]).add(
		controlPoints.get(3).get(1).mul(bus[1]).add(
		controlPoints.get(3).get(2).mul(bus[2])).add(
		controlPoints.get(3).get(3).mul(bus[3]))).mul(bvs[3]);
	...
	public static IDeform makeFFD(double fadeDist, Double[] gridX, Double[] gridY,
			Vec[][] cps) {
		List<List<Vec>> controls = new ArrayList<List<Vec>>();
		for(Vec[] cp : cps){
			controls.add(Arrays.asList(cp));
		}
		return new CubicFFD(fadeDist,Arrays.asList(gridX), Arrays.asList(gridY), controls);
	}
