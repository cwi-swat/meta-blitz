...
public class CubicFFDPadder {
	...
	List<List<Vec>> getLeftPadding(){
		...
		for(int y = 0 ; y < controlPoints.size(); y++){
			mirr.add(controlPoints.get(y).get(0).mirror(controlPoints.get(y).get(1)));
		}
		res.add(mirr);
	...
	List<List<Vec>> getRightPadding(){
		...
		for(int y = 0 ; y < controlPoints.size(); y++){
			...
			mirr.add(controlRow.get(controlRow.size()-1).mirror(controlRow.get(controlRow.size()-2)));
		}
		res.add(mirr);
		res.add(makeWithYes(gridX.get(gridX.size()-1) + (2.0/3.0)*fadeDist));
	...
	List<List<Vec>> getBottomPadding(){
		...
		res.add(mirr);
		res.add(makeWithXes(gridY.get(gridY.size()-1) + (2.0/3.0)*fadeDist));
