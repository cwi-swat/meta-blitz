...
public class ColorAlpha implements ISample<ColorAlpha>{
	...
	public ColorAlpha mul(int d) {
		d = clamp(d);
		return new ColorAlpha((r*d >> 8) , (g*d >> 8) , (b*d >> 8), (a*d) >> 8);
	}
	...
	public ColorAlpha add(ColorAlpha rhs) {
		return new ColorAlpha(clamp(r + rhs.r), clamp(g + rhs.g) , clamp(b + rhs.b), clamp(a + rhs.a) );
	}
	...
	public ColorAlpha read(DataBuffer data, int index) {
		return  new ColorAlpha(data.getElem(index + 3), data.getElem(index + 2), data.getElem(index + 1), data.getElem(index));
	}
