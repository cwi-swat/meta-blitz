...
public abstract class SplittableCompound<LSimp extends Path,RSimp extends Path> extends Path<CompoundSplitIndex,LSimp,RSimp> {
	...
	public Vec getAt(CompoundSplitIndex t) {
		switch(t.choice){
		case Left: return left.getAt(t.next);
		case Right: return right.getAt(t.next);
		}
	...
	public Vec getTangentAt(CompoundSplitIndex t) {
		switch(t.choice){
		case Left: return left.getTangentAt(t.next);
		case Right: return right.getTangentAt(t.next);
		}
		throw new Error("Unkown split choice:" + this + "\n");
	}
	...
	private List<CompoundSplitIndex> prependList(SplitChoice choice,List<? extends PathIndex> r) {
	...
	private <LPI> Tuple<List<LPI>, List<CompoundSplitIndex>> prependListRhs(
			SplitChoice choice,Tuple<List<LPI>, List<? extends PathIndex>> intersections) {
	...
	private <LPI> BestProjectTup<LPI, CompoundSplitIndex> prependBestTupRhs(
			SplitChoice choice, BestProjectTup<LPI, ? extends PathIndex> projectSimplerTup){
	...
	private <LPI>  BestProjectTup<CompoundSplitIndex, LPI>  prependBestTupLhs(
			SplitChoice choice, BestProjectTup<? extends PathIndex, LPI> projectSimplerTup){
		if(projectSimplerTup.t == null){
			return (BestProjectTup)projectSimplerTup;
		} else {
			return new BestProjectTup<CompoundSplitIndex,LPI>(
					projectSimplerTup.distSquared, new CompoundSplitIndex(choice,projectSimplerTup.t.l), 
