...
public final class AffineTransformation {
	...
	public AffineTransformation rotate(double rads){
		return new AffineTransformation(Matrix.rotate(rads).mul(to), back.mul(Matrix.rotate(-rads)));
	}
	
	public AffineTransformation translate(double x,double y){
		return new AffineTransformation(Matrix.translate(x, y).mul(to), back.mul(Matrix.translate(-x,-y)));
	}
	...
	public AffineTransformation scale(double x, double y){
		return new AffineTransformation(Matrix.scale(x, y).mul(to), back.mul(Matrix.scale(1.0/x,1.0/y)));
	}
	...
	public AffineTransformation shearX(double x){
		return new AffineTransformation(Matrix.shearX(x).mul(to), back.mul(Matrix.shearX(-x)));
	}
